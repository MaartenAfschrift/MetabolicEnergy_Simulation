...
function [setup,auxdata,DatStore]=GetFtildeSetup(model_path,IK_path,ID_path,time,OutPath,Misc)
...
% Linear tendon ?
if ~isfield(Misc,'BoolLinearSpring') || isempty(Misc.BoolLinearSpring)
    Misc.BoolLinearSpring=1;
end
...
disp('Only 30% of ID torque generated by muscle'); DatStore.T_exp=DatStore.T_exp.*0.3;
...
% Linear tendon ?
auxdata.BoolLinearSpring=Misc.BoolLinearSpring;
...
splinestruct = SplineInputData_vM(tdummy,input);
...
setup.functions.continuous = @Wrap4musdynContinous_FtildeState_vM1;
